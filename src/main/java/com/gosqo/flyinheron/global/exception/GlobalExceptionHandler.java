...
public class GlobalExceptionHandler {
    ...
    public <E extends Exception> ResponseEntity<ErrorResponse> delegateHandling(
        ...
        if (e instanceof ExpiredJwtException) {
            return buildResponseEntity(HttpStatus.UNAUTHORIZED, "회원이라면 잠시 후 다시 시도해주세요.");
        }

        if (e instanceof BadCredentialsException) {
            return buildResponseEntity(HttpStatus.BAD_REQUEST, "이메일 혹은 패스워드를 확인 해주세요.");
        }

        if (e instanceof AccessDeniedException) {
            return buildResponseEntity(HttpStatus.FORBIDDEN, "로그인 후 이용해주시기 바랍니다.");
        }

        if (e instanceof AuthenticationException) {
            return buildResponseEntity(HttpStatus.FORBIDDEN, "인증 정보에 문제가 있습니다.");
        }
        ...
        if (e instanceof DataIntegrityViolationException) {
            return buildResponseEntity(HttpStatus.CONFLICT, "자원의 입력 및 수정이 중복을 발생시킵니다. 이 중복은 허용되지 않습니다.");
        }

        if (e instanceof NoSuchElementException
                || e instanceof NoResourceFoundException
        ) {
            return buildResponseEntity(HttpStatus.NOT_FOUND, "존재하지 않는 자원에 대한 요청입니다.");
        }

        if (e instanceof IllegalArgumentException
                || e instanceof MethodArgumentTypeMismatchException
        ) {
            return buildResponseEntity(HttpStatus.BAD_REQUEST, "올바르지 않은 요청입니다.");
        }
        ...
        if (e instanceof HttpRequestMethodNotSupportedException) {
            ...
            return buildResponseEntity(HttpStatus.METHOD_NOT_ALLOWED, "API 사용에 합의되지 않은 요청입니다.");
        }

        if (e instanceof RuntimeException) {
            return buildResponseEntity(HttpStatus.INTERNAL_SERVER_ERROR, "서버 오류가 발생했습니다. 문제가 지속될 시 운영진에 연락 부탁드립니다.");
        }

        return buildResponseEntity(HttpStatus.INTERNAL_SERVER_ERROR, "서버 오류가 발생했습니다.");
    }
